...
2. **PDF (Overview & Monthly Report):**
   - Content: Username, Title, Date Range (Current Month, Last 30 Days, or custom historical Month/Year selection bounded between 2021 and current date), Applied Filters, Balance Summary (net balance styled in green if > 0, red if < 0, neutral black if exactly 0), Result Graphs, Transactions List with colored Type badges (Income in green, Expense in red, Transfer neutral), Multi-line centered Transfer routes (`Origin\n↓\nDestination`), Budgets & Savings Goals breakdown, and Generation Date.
3. **JSON Backup:**
