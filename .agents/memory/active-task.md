...
## Current Sub-tasks
- [ ] **Task 7.1: Profile Setup Flow.** Update `auth_screen` to handle account creation (4-8 digit PIN, language selection, terms acceptance).
- [ ] **Task 7.2: Biometric Integration.** Prompt for biometric authentication after PIN creation. Fallback to PIN if rejected or unavailable.
- [ ] **Task 7.3: Default Wallet Creation.** Automatically create an account named "Mi cartera" (localized) with 0 balance upon successful profile creation.
- [ ] **Task 7.4: UI Empty States.** Integrate `EmptyStateWidget` into Dashboard, Transactions, and Accounts lists when no data is available.
