...
class ExecuteRecurringTransactionsUseCase {
  ...
  Future<void> execute() async {
    ...
    final todayUtcPlus2 = DateTime.utc(nowUtcPlus2.year, nowUtcPlus2.month, nowUtcPlus2.day);

    debugPrint('[ExecuteRecurringTxns] Starting evaluation. Today in UTC+2 is: $todayUtcPlus2');
    ...
    for (final autoTxn in automaticTxns) {
      ...
      if (nextDateUtcPlus2.isBefore(todayUtcPlus2) || nextDateUtcPlus2.isAtSameMomentAs(todayUtcPlus2)) {
        try {
          await _fireTransaction(autoTxn, nowUtc, todayUtcPlus2);
          fired++;
        } catch (e, st) {
          debugPrint('[ExecuteRecurringTxns] ERROR firing "${autoTxn.name}": $e\n$st');
        }
      } else {
        skipped++;
      }
  ...
  Future<void> _fireTransaction(AutomaticTransaction autoTxn, DateTime nowUtc, DateTime executionDateUtcPlus2) async {
    final account = await accountRepo.getAccountById(autoTxn.accountId);
    final profile = account != null ? await profileRepo.getProfileById(account.userId) : null;
    ...
    try {
      final localRates = await exchangeRateRepo.getLocalRates(baseCurrency: defaultCurrency);
      if (localRates != null) {
    ...
    if (originalCurrency != defaultCurrency && exchangeRate == null) {
      try {
        final remoteRates = await exchangeRateRepo.getLatestRates(baseCurrency: defaultCurrency);
    ...
    final nextDate = calculateNextTriggerDateUtcPlus2(autoTxn.nextExecutionDate, autoTxn.recurrenceType, autoTxn.recurrenceDays);
  ...
  static DateTime calculateNextTriggerDateUtcPlus2(DateTime fromDate, RecurrenceType type, int recurrenceDays) {
  ...
  static DateTime _advanceByYears(DateTime from, int years, int targetMonth, int targetDay) {
