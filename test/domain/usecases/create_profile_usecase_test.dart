...
void main() {
  ...
  setUp(() {
    ...
    usecase = CreateProfileUseCase(mockProfileRepository, mockSecureStorageManager);
  });
  ...
  group('CreateProfileUseCase Unit Tests', () {
    test('should successfully hash PIN, save it, and create a new profile when database is empty', () async {
      // Arrange
      final expectedPinHash = calculateHash('1234');
      when(() => mockSecureStorageManager.savePinHash(any())).thenAnswer((_) async {});
      when(() => mockProfileRepository.getFirstProfile()).thenAnswer((_) async => null);
      ...
      verify(() => mockSecureStorageManager.savePinHash(expectedPinHash)).called(1);
    ...
    test('should update existing profile (retaining its ID) when profile already exists in database', () async {
      // Arrange
      ...
      final expectedPinHash = calculateHash('1234');
      when(() => mockSecureStorageManager.savePinHash(any())).thenAnswer((_) async {});
      when(() => mockProfileRepository.getFirstProfile()).thenAnswer((_) async => existingProfile);
      ...
      // Assert
      expect(result.id, existingProfile.id); // Must retain the same ID to protect references
      expect(result.name, defaultParams.name);
      expect(result.username, defaultParams.username);
      expect(result.defaultCurrency, defaultParams.defaultCurrency);
    ...
    test('should throw ValidationException when PIN is too short (< 4 digits)', () async {
      // Arrange
    ...
    test('should throw ValidationException when PIN is too long (> 8 digits)', () async {
      // Arrange
      final params = CreateProfileParams(
        name: 'Carles',
        username: 'carlespv',
    ...
    test('should throw ValidationException when PIN contains non-numeric characters', () async {
      // Arrange
      final params = CreateProfileParams(
        name: 'Carles',
        username: 'carlespv',
