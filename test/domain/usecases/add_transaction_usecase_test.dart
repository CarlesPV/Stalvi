...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => mockFinancialThresholdService.evaluateThresholds(any()),
    ).thenAnswer((_) async => []);
  ...
  group('AddTransactionUseCase', () {
    ...
    group('income/expense — success cases', () {
      test(
        'creates transaction without conversion when currencies match',
        () async {
          ...
          when(
            () => mockAccountRepo.getAccountById(params.accountId),
          ).thenAnswer((_) async => account);
          when(
            () => mockProfileRepo.getProfileById(account.userId),
      ...
      test(
        ...
        () async {
          ...
          final account = _buildAccount();
          final profile = _buildProfile();
          final localRates = _buildExchangeRate(
            base: 'EUR',
      ...
      test('calculates convertedAmount when currencies differ', () async {
        ...
        final rateSnapshot = _buildExchangeRate(
          base: 'EUR',
        ); // 1 EUR = 1.08 USD
    ...
    group('transfer — success cases', () {
      test(
        'creates TWO mirrored transactions via createTransferPair',
        () async {
          ...
          when(
            () => mockAccountRepo.getAccountById('account_1'),
          ).thenAnswer((_) async => originAccount);
          when(
            () => mockAccountRepo.getAccountById('account_2'),
          ).thenAnswer((_) async => destAccount);
          when(
            () => mockProfileRepo.getProfileById(originAccount.userId),
          ).thenAnswer((_) async => profile);
          ...
          when(
            ...
            (_) async =>
                TransferPair(origin: fakeOrigin, destination: fakeDest),
          );
      ...
      test(
        'origin leg has origin accountId, destination has dest accountId',
        () async {
          final params = _transferParams();
          final originAccount = _buildAccount(id: 'account_1');
          final destAccount = _buildAccount(id: 'account_2', isDefault: false);
    ...
    group('transfer — validation failures', () {
      ...
      test(
        'throws NotFoundException when destination account does not exist',
        () async {
          final params = _transferParams();
          final originAccount = _buildAccount(id: 'account_1');
          final profile = _buildProfile();
    ...
    group('common — failure cases', () {
      test(
        'throws ValidationException when category is null for income/expense',
        () async {
          final params = AddTransactionParams(
          ...
          when(
            () => mockAccountRepo.getAccountById(any()),
          ).thenAnswer((_) async => account);
          when(
            () => mockProfileRepo.getProfileById(any()),
          ).thenAnswer((_) async => profile);
      ...
      test('throws ValidationException when date in future', () async {
        final call = usecase.execute(
          _incomeParams(date: _now.add(const Duration(days: 1))),
        );
        await expectLater(() => call, throwsA(isA<ValidationException>()));
      });
      ...
      test(
        'throws ValidationException when rate not found for currency',
        () async {
          final params = _incomeParams();
    ...
    group('threshold push notifications', () {
      ...
      test(
        ...
        () async {
          ...
          when(
            () => mockFinancialThresholdService.evaluateThresholds(any()),
          ).thenAnswer((_) async => [ThresholdResult(isBudgetExceeded: true)]);
      ...
      test(
        'triggers showGoalReachedNotification when savings goal is reached',
        () async {
          final params = _incomeParams();
          final account = _buildAccount();
          final profile = _buildProfile();
          ...
          when(
            ...
            (_) async => [ThresholdResult(isSavingsGoalReached: true)],
          );
