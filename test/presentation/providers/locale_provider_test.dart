...
void main() {
  ...
  group('LocaleNotifier Unit Tests', () {
    test('defaults to system locale or en when secure storage is empty', () async {
    ...
    test('loads persisted locale from secure storage if present', () async {
      when(() => mockSecureStorage.getUserLocale()).thenAnswer((_) async => 'es');
    ...
    test('setLocale updates state and persists to secure storage', () async {
      when(() => mockSecureStorage.getUserLocale()).thenAnswer((_) async => null);
      when(() => mockSecureStorage.setUserLocale('ca')).thenAnswer((_) async {});
