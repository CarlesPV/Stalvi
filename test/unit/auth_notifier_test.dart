...
void main() {
  ...
  group('AuthNotifier build()', () {
    test('returns unauthenticated when PIN exists and no lockout', () async {
      ...
      final container =
          _buildContainer(storage: mockStorage, biometrics: mockBiometrics);
    ...
    test('grants 1 extra attempt when lockout expired while app was closed',
        () async {
      final nowMs = DateTime.now().millisecondsSinceEpoch;
      when(mockStorage.hasPin()).thenAnswer((_) async => true);
  ...
  group('AuthNotifier.verifyPin() — correct PIN', () {
    ...
    test('resets remainingPinAttempts to 5 after successful verification',
        () async {
      const goodPin = '1234';
      const badPin = '0000';
  ...
  group('AuthNotifier.verifyPin() — wrong PIN brute-force protection', () {
    ...
    test('pinLockoutSecondsRemaining equals 30 immediately after lockout',
        () async {
      _stubNoPinLockout(mockStorage, pinHash: _hash('correct'));
      final container =
          _buildContainer(storage: mockStorage, biometrics: mockBiometrics);
  ...
  group('AuthNotifier lockout countdown and extra attempt logic', () {
    test('transitions to unauthenticated with 1 remaining attempt after 30 s',
        () async {
      _stubNoPinLockout(mockStorage, pinHash: _hash('correct'));
      final container =
          _buildContainer(storage: mockStorage, biometrics: mockBiometrics);
    ...
    test('wrong entry after grace attempt triggers a second 30 s lockout',
        () async {
      _stubNoPinLockout(mockStorage, pinHash: _hash('correct'));
      final container =
          _buildContainer(storage: mockStorage, biometrics: mockBiometrics);
